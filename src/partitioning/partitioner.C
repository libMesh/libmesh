...
void Partitioner::set_interface_node_processor_ids_linear(MeshBase & mesh)
{
  ...
  // If we don't see the whole mesh, then we should be correct about
  // our own and our paired processors' nodes at this point but we may
  // need to sync up information about others.
  if (!mesh.is_serial())
    {
      set_nearly_remote_node_processor_ids(mesh);
...
void Partitioner::set_node_processor_ids(MeshBase & mesh)
{
  ...
  // Select how we try to improve our node pid settings
  ...
  // Or just make sure we didn't miss any node PIDs
  else
    set_nearly_remote_node_processor_ids(mesh);
...
void Partitioner::set_nearly_remote_node_processor_ids(MeshBase & mesh)
{
  // On a replicated or serialized mesh we shouldn't have any "nearly
  // remote" nodes; every node should have every element attached to
  // it visible.
  if (mesh.is_serial())
    return;
