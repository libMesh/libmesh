  ...
  for (const auto * elem : mesh.active_local_element_ptr_range())
    {
      ...
      if (_target_jacobians.find(elem->type()) == _target_jacobians.end())
        {
          const auto [target_elem, target_nodes] = ReferenceElem::ideal_target(elem->type());
  ...
  for (const auto * elem : mesh.active_local_element_ptr_range())
    {
      ...
      if (det_S_int < info.min_elem_det_S.first)
      ...
      if (beta_int < info.min_elem_distortion.first)
      ...
      if (mu_int < info.min_elem_dilation.first)
