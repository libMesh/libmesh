...
p2t::Point
to_p2t(const libMesh::Point & p)
{
#if LIBMESH_DIM > 2
  libmesh_error_msg_if(p(2) != 0, "Poly2TriTriangulator only supports point sets in the XY plane");
#endif
...
Real
distance_from_circumcircle(const Elem & elem, const Point & p)
{
...
bool
in_circumcircle(const Elem & elem, const Point & p, const Real tol = 0)
{
...
can_delaunay_swap(const Elem & elem, unsigned short side, Real tol)
{
  ...
  for (; nn < 3; ++nn)
  {
    ...
    if (neigh_node == elem.node_ptr(0) || neigh_node == elem.node_ptr(1) ||
  ...
  const RealVectorValue right = (elem.point((n + 1) % 3) - elem.point(n)).unit();
  const RealVectorValue mid = (neigh->point(nn) - elem.point(n)).unit();
  const RealVectorValue left = (elem.point((n + 2) % 3) - elem.point(n)).unit();
  ...
  if (mid * right < left * right || left * mid < left * right)
    return {false, 0};
...
[[maybe_unused]] void
libmesh_assert_locally_delaunay(const Elem & elem)
{
  libmesh_ignore(elem);
...
inline void
libmesh_assert_delaunay(MeshBase & libmesh_dbg_var(mesh), Container & new_elems)
{
...
template <typename Container>
inline void
restore_delaunay(Container & check_delaunay_on, BoundaryInfo & boundary_info)
{
  ...
  while (!check_delaunay_on.empty())
  {
    ...
    for (auto s : make_range(elem.n_sides()))
    {
      ...
      // back and forth.
      auto [can_swap, nn] = can_delaunay_swap(elem, s, -TOLERANCE * TOLERANCE);
      if (!can_swap)
        continue;
...
unsigned int
segment_intersection(const Elem & elem,
                     Point & source,
                     const Point & target,
                     unsigned int source_side)
{
  ...
  for (auto s : make_range(ns))
  {
    ...
    // along each segment)
    const Real raydx = target(0) - source(0), raydy = target(1) - source(1), edgedx = v1(0) - v0(0),
               edgedy = v1(1) - v0(1);
    ...
    const Real targetsdx = v1(0) - target(0), targetsdy = v1(1) - target(1);

    const Real t_num = targetsdx * raydy - targetsdy * raydx;
...
// Constructor
Poly2TriTriangulator::Poly2TriTriangulator(UnstructuredMesh & mesh, dof_id_type n_boundary_nodes)
  : TriangulatorInterface(mesh), _n_boundary_nodes(n_boundary_nodes), _refine_bdy_allowed(true)
{
}
...
Poly2TriTriangulator::triangulate()
{
  ...
  // ever.
  if (_elem_type != TRI3 && _elem_type != TRI6 && _elem_type != TRI7)
    libmesh_not_implemented();
...
void
Poly2TriTriangulator::set_desired_area_function(FunctionBase<Real> * desired)
{
  if (desired)
...
FunctionBase<Real> *
Poly2TriTriangulator::get_desired_area_function()
{
  return _desired_area_func.get();
}

bool
Poly2TriTriangulator::is_refine_boundary_allowed(const BoundaryInfo & boundary_info,
                                                 const Elem & elem,
                                                 unsigned int side)
{
...
void
Poly2TriTriangulator::triangulate_current_points()
{
  ...
  libmesh_error_msg_if(!nn, "Poly2TriTriangulator cannot triangulate an empty mesh!");

  // Unless we're using an explicit segments list, we assume node ids
  // are contiguous here.
  if (this->segments.empty())
    libmesh_error_msg_if(_mesh.n_nodes() != nn,
  ...
  if (_n_boundary_nodes == DofObject::invalid_id)
  {
    ...
    libmesh_assert_equal_to(std::ptrdiff_t(_n_boundary_nodes),
                            std::distance(_mesh.nodes_begin(), _mesh.nodes_end()));
  }
  else
    libmesh_assert_less_equal(_n_boundary_nodes, _mesh.n_nodes());
  ...
  else
  {
    ...
    for (auto [segment_start, segment_end] : this->segments)
    {
      if (last_id != DofObject::invalid_id)
        libmesh_error_msg_if(segment_start != last_id, "Disconnected triangulator segments");
      last_id = segment_end;

      Node * node = _mesh.query_node_ptr(segment_start);

      libmesh_error_msg_if(!node,
                           "Triangulator segments reference nonexistent node id " << segment_start);
      ...
      // We're not going to support overlapping nodes on the boundary
      if (point_node_map.count(*pt))
        libmesh_not_implemented_msg("Triangulating overlapping boundary nodes is unsupported");
    ...
    {
      const p2t::Point pt = to_p2t(*node);
      if (const auto it = point_node_map.find(pt); it == point_node_map.end())
      {
        steiner_points.insert(pt);
        point_node_map.emplace(pt, node);
      }
  ...
  std::unordered_map<std::pair<dof_id_type, dof_id_type>, boundary_id_type, libMesh::hash>
      side_boundary_id;
  ...
  {
    const Node *node1 = libmesh_map_find(point_node_map, outer_boundary_points[i]),
               *node2 = libmesh_map_find(point_node_map, outer_boundary_points[(i + 1) % n_outer]);

    side_boundary_id.emplace(std::make_pair(node1->id(), node2->id()), outer_bcid);
  }
  ...
  for (auto h : make_range(n_holes))
  {
    ...
    const Hole & our_hole = (it == replaced_holes.end()) ? *initial_hole : *it->second;
    ...
    for (auto i : make_range(our_hole.n_points()))
    {
      ...
      if (point_node_map.count(pt))
      {
        libmesh_assert_equal_to(point_node_map[pt], _mesh.query_node_ptr(point_node_map[pt]->id()));
      }
      else
      {
    ...
    {
      const Node *node1 = libmesh_map_find(point_node_map, poly2tri_hole[i]),
                 *node2 = libmesh_map_find(point_node_map, poly2tri_hole[(i + 1) % n_inner]);

      side_boundary_id.emplace(std::make_pair(node1->id(), node2->id()), inner_bcid);
    }
  ...
  for (auto ptri_ptr : triangles)
  {
    ...
    for (auto v : make_range(3))
    {
      const Node &node1 = added_elem->node_ref(v), &node2 = added_elem->node_ref((v + 1) % 3);
...
bool
Poly2TriTriangulator::insert_refinement_points()
{
  ...
  // is expensive!
  {
    ...
    // carefully.
    std::set<Elem *, decltype(comp)> all_elems{mesh.elements_begin(), mesh.elements_end(), comp};
  ...
  if (this->desired_area() == 0 && this->get_desired_area_function() == nullptr &&
  ...
  for (auto & elem : mesh.element_ptr_range())
  {
    ...
    auto boundary_refine = [this, &next_boundary_node, &cavity_elem, &new_node](unsigned int side)
    {
      ...
      if (auto it = next_boundary_node.find(*old_segment_start); it != next_boundary_node.end())
      {
      ...
      else
      {
        ...
        libmesh_assert(!this->segments.empty() || (_holes && !_holes->empty()) ||
                       (old_segment_end->id() == old_segment_start->id() + 1));
    ...
    while (!cavity_elem->contains_point(new_pt))
    {
      ...
      if (!neigh)
      {
        if (this->is_refine_boundary_allowed(boundary_info, *cavity_elem, side))
        {
          new_pt = ray_start;
    ...
    if (side == invalid_uint && !new_node)
    {
      ...
      for (auto s : make_range(3u))
      {
        ...
        Real ax = cavity_elem->point(s)(0) - new_pt(0), ay = cavity_elem->point(s)(1) - new_pt(1),
             bx = cavity_elem->point((s + 1) % 3)(0) - new_pt(0),
             by = cavity_elem->point((s + 1) % 3)(1) - new_pt(1);
        const Real my_cos =
            (ax * bx + ay * by) / std::sqrt(ax * ax + ay * ay) / std::sqrt(bx * bx + by * by);
      ...
      if (worst_cos < -0.6) // -0.5 is the best we could enforce?
      {
        ...
        if (this->is_refine_boundary_allowed(boundary_info, *cavity_elem, side))
        {
          ...
          new_pt = (cavity_elem->point(side) + cavity_elem->point((side + 1) % 3)) / 2;
          new_node = add_point(new_pt);
          boundary_refine(side);
        }
    ...
    std::unordered_map<Node *, std::pair<Elem *, boundary_id_type>> neighbors_CCW, neighbors_CW;
    ...
    for (Elem * old_elem : cavity)
    {
      for (auto s : make_range(3u))
      {
        ...
        {
          Node *node_CW = old_elem->node_ptr(s), *node_CCW = old_elem->node_ptr((s + 1) % 3);

          auto set_neighbors = [&neighbors_CW, &neighbors_CCW, &node_CW, &node_CCW, &boundary_info](
                                   Elem * new_neigh, boundary_id_type bcid)
          {
            // Set clockwise neighbor and vice-versa if possible
            if (const auto CW_it = neighbors_CW.find(node_CW); CW_it == neighbors_CW.end())
            {
            ...
            if (const auto CCW_it = neighbors_CCW.find(node_CCW); CCW_it == neighbors_CCW.end())
            {
          ...
          if (old_elem == cavity_elem && s == side)
          {
            std::vector<boundary_id_type> bcids;
          ...
          if (neigh)
          {
            const unsigned int neigh_s = neigh->which_neighbor_am_i(old_elem);
    ...
    for (Elem * old_elem : cavity)
    {
      if (const auto it = new_elems.find(old_elem); it == new_elems.end())
  ...
  {
    auto checked_emplace = [this](dof_id_type new_first, dof_id_type new_second)
    {
#ifdef DEBUG
    ...
    if (this->segments.empty())
    {
      ...
      for (auto node_it = _mesh.nodes_begin(), node_end = _mesh.nodes_end(); node_it != node_end;)
      {
        Node & node = **node_it;
        ++node_it;
        ...
        if (last_id != DofObject::invalid_id && last_id != node_id)
        ...
        while (it != next_boundary_node.end())
        {
          ...
          if (node_it != node_end && next_node == *node_it)
            ++node_it;
      ...
      if (this->segments.back().second != this->segments.front().first)
        checked_emplace(this->segments.back().second, this->segments.front().first);
    }
    else
    {
      ...
      {
        const dof_id_type node_id = node->id();
        if (const auto it = next_boundary_node.find(*node); it == next_boundary_node.end())
        {
    ...
    if (this->_holes)
    {
      ...
      for (const Hole * hole : *this->_holes)
      {
        ...
        if (hole_point_insertion)
          this->replaced_holes.emplace(hole, std::make_unique<ArbitraryHole>(*hole));
      }
      ...
      for (const Hole * hole : *this->_holes)
      {
        ...
        {
          // O(1) assert in devel
          libmesh_assert(new_points.empty() || new_points.back() != p);
#ifdef DEBUG
          // O(N) asserts in dbg
        ...
        for (auto point_it = arb.get_points().rbegin(), point_end = arb.get_points().rend();
             point_it != point_end;)
        {
          Point point = *point_it;
          ++point_it;

          if (new_points.empty() || (point != new_points.back() && point != new_points.front()))
          ...
          while (it != next_boundary_node.end())
          {
            ...
            if (point == new_points.front())
              break;
            if (point_it != point_end && point == *point_it)
              ++point_it;
            push_back_new_point(point);
...
bool
Poly2TriTriangulator::should_refine_elem(Elem & elem)
{
  const Real min_area_target = this->desired_area();
  FunctionBase<Real> * area_func = this->has_auto_area_function()
                                       ? this->get_auto_area_function()
                                       : this->get_desired_area_function();

  // If this isn't a question, why are we here?
  libmesh_assert(min_area_target > 0 || area_func != nullptr || this->has_auto_area_function());
  ...
  if (!area_func && !this->has_auto_area_function())
    ...
    libmesh_warning("WARNING:  both desired are function and automatic area function are set.  "
                    "Using automatic area function.");
  ...
  for (auto v : make_range(elem.n_vertices()))
  {
    ...
    libmesh_error_msg_if(local_area_target <= 0,
