                                             ...
                                             dof_id_type coarse_id)
{
  ...
  for (int side : make_range(n_sides))
    {
      ...
      // We might need to ask the owner of a ghost element about
      // whether it saw any refinement forced by a coarse neighbor
      // with a smaller desired area
  ...
  if (auto it = new_nodes.find(vertices_to_refine);
    ...
    {
      // Add with no processor id first, so we'll get a temporary
      // id that won't conflict with any canonical ids later
      midedge_node =
        _mesh.add_point((*(Point*)vertices_to_refine.first +
                         *(Point*)vertices_to_refine.second)/2);

      // Give the new node the best pid we can.
  ...
  // Preserve boundary and remote_elem neighbor information
  ...
  if (elem.neighbor_ptr(side_to_refine) == remote_elem)
    {
      subelem[0]->set_neighbor(0, const_cast<RemoteElem*>(remote_elem));
      subelem[1]->set_neighbor(1, const_cast<RemoteElem*>(remote_elem));
    }

  boundary_info.boundary_ids(&elem, (side_to_refine+1)%3, bc_ids);
  boundary_info.add_side(subelem[1].get(), 2, bc_ids);
  if (elem.neighbor_ptr((side_to_refine+1)%3) == remote_elem)
    subelem[1]->set_neighbor(2, const_cast<RemoteElem*>(remote_elem));

  boundary_info.boundary_ids(&elem, (side_to_refine+2)%3, bc_ids);
  boundary_info.add_side(subelem[0].get(), 2, bc_ids);
  if (elem.neighbor_ptr((side_to_refine+2)%3) == remote_elem)
    subelem[0]->set_neighbor(2, const_cast<RemoteElem*>(remote_elem));
  ...
  for (unsigned int i=0; i != max_subelems; ++i)
    {
      added_elements[coarse_id].emplace(subelem[i]->vertex_average(),
                                        subelem[i].get());
      ...
      for (auto & elem : _mesh.element_ptr_range())
        {
          newly_refined_elements +=
            this->refine_via_edges(*elem, elem->id());
        }
      ...
      auto added_action_functor =
         ...
         const std::vector<elem_refinement_datum> & coarse_refinements)
        {
          ...
          for (auto i : make_range(query_size))
            {
              const dof_id_type coarse_id = coarse_elems[i];
              const auto & refinement_data = coarse_refinements[coarse_id];
