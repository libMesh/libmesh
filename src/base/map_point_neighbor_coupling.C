...
void MapPointNeighborCoupling::mesh_reinit()
{
...
void MapPointNeighborCoupling::operator()
  (const MeshBase::const_element_iterator & range_begin,
   const MeshBase::const_element_iterator & range_end,
   processor_id_type p,
   map_type & coupled_elements)
{
  LOG_SCOPE("operator()", "MapPointNeighborCoupling");

  if (_nodes_to_elem_map.empty())
    MeshTools::build_nodes_to_elem_map(*_mesh, _nodes_to_elem_map);
      ...
      for (const auto & elem : elements_to_check)
        {
          ...
          {
            auto & point_neighbors = _nodes_to_elem_map[node.id()];
