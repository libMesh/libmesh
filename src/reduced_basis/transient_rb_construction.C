...
TransientRBConstruction::TransientRBConstruction (EquationSystems& es,
  ...
  : Parent(es, name_in, number_in),
    L2_matrix(SparseMatrix<Number>::build(es.communicator())),
    non_dirichlet_L2_matrix(SparseMatrix<Number>::build(es.communicator())),
...
void TransientRBConstruction::allocate_data_structures()
{
  ...
  if(!single_matrix_mode)
  {
    ...
    for(unsigned int q=0; q<Q_m; q++)
    {
      ...
      M_q_vector[q] = SparseMatrix<Number>::build(this->communicator()).release();
    ...
    if(store_non_dirichlet_operators)
    {
      ...
      {
        // Initialize the memory for the matrices
        non_dirichlet_M_q_vector[q] = SparseMatrix<Number>::build(this->communicator()).release();
  ...
  {
    temporal_data[i] = (NumericVector<Number>::build(this->communicator()).release());
...
void TransientRBConstruction::assemble_affine_expansion()
{
  ...
  if(get_rb_evaluation().get_n_basis_functions() > 0)
  {
    ...
    AutoPtr< NumericVector<Number> > temp1 = NumericVector<Number>::build(this->communicator());
...
void TransientRBConstruction::add_IC_to_RB_space()
{
  ...
  get_rb_evaluation().basis_functions.push_back( NumericVector<Number>::build(this->communicator()).release() );
...
void TransientRBConstruction::update_residual_terms(bool compute_inner_products)
{
  ...
  for(unsigned int q_m=0; q_m<Q_m; q_m++)
  {
    for(unsigned int i=(RB_size-delta_N); i<RB_size; i++)
    {
      ...
      {
        trans_rb_eval.M_q_representor[q_m][i] = (NumericVector<Number>::build(this->communicator()).release());
...
void TransientRBConstruction::update_RB_initial_condition_all_N()
{
  ...
  AutoPtr< NumericVector<Number> > temp2 = NumericVector<Number>::build(this->communicator());
                                                           ...
                                                           const bool read_binary_residual_representors)
{
  ...
  for (unsigned int i=0; i<trans_rb_eval.M_q_representor.size(); ++i)
	...
	trans_rb_eval.M_q_representor[i][j] = NumericVector<Number>::build(this->communicator()).release();
