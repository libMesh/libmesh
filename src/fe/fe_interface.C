...
RealGradient FEInterface::vectorshape_hcurl(const unsigned int dim,
                                            const FEType & fe_t,
                                            const Elem * elem,
                                            const unsigned int i,
                                            const Point & p)
{
  auto fe = FEVectorBase::build(dim, fe_t);
  std::vector<Point> pt = {p};
  fe->get_dphideta(); // to compute the map
  fe->reinit(elem, &pt);

  // We are computing mapping shape functions, so we explicitly ignore
  // any non-zero p_level() the Elem might have.
  RealGradient vector_shape = vectorshape(dim, fe_t, elem, i, p);
  RealGradient phi;
  ...
  case 2:
    {
      const Real & dxidx = fe->get_dxidx()[0];
      const Real & dxidy = fe->get_dxidy()[0];
      const Real & dxidz = fe->get_dxidz()[0];

      const Real & detadx = fe->get_detadx()[0];
      const Real & detady = fe->get_detady()[0];
      const Real & detadz = fe->get_detadz()[0];
      ...
      phi(0) = dxidx*vector_shape(0) + detadx*vector_shape(1);
  ...
  case 3:
    {
      ...
      const Real & dzetadx = fe->get_dzetadx()[0];
      const Real & dzetady = fe->get_dzetady()[0];
      const Real & dzetadz = fe->get_dzetadz()[0];
      ...
      phi(0) = dxidx*vector_shape(0) + detadx*vector_shape(1)
        + dzetadx*vector_shape(2);

      phi(1) = dxidy*vector_shape(0) + detady*vector_shape(1)
        + dzetady*vector_shape(2);
...
RealGradient FEInterface::vectorshape_hdiv(const unsigned int dim,
                                           const FEType & fe_t,
                                           const Elem * elem,
                                           const unsigned int i,
                                           const Point & p)
{
  ...
  switch (dim)
    ...
    case 2:
      {
        const RealGradient & dxyz_dxi   = fe->get_dxyzdxi()[0];
        const RealGradient & dxyz_deta  = fe->get_dxyzdeta()[0];

        const Real & J = fe->get_fe_map().get_jacobian()[0];

        // phi = J^{-1} * (dx/dxi) * \hat{phi}
        Real dx_dxi   = dxyz_dxi(0);
        Real dx_deta  = dxyz_deta(0);

        Real dy_dxi   = dxyz_dxi(1);
        Real dy_deta  = dxyz_deta(1);

        Real dz_dxi   = dxyz_dxi(2);
        Real dz_deta  = dxyz_deta(2);

        phi(0) = (dx_dxi*vector_shape(0) + dx_deta*vector_shape(1))/J;
        phi(1) = (dy_dxi*vector_shape(0) + dy_deta*vector_shape(1))/J;
        phi(2) = (dz_dxi*vector_shape(0) + dz_deta*vector_shape(1))/J;
    ...
    case 3:
      {
        const RealGradient & dxyz_dxi   = fe->get_dxyzdxi()[0];
        const RealGradient & dxyz_deta  = fe->get_dxyzdeta()[0];
        const RealGradient & dxyz_dzeta = fe->get_dxyzdzeta()[0];
        ...
        Real dx_dxi   = dxyz_dxi(0);
        Real dx_deta  = dxyz_deta(0);
        Real dx_dzeta = dxyz_dzeta(0);

        Real dy_dxi   = dxyz_dxi(1);
        Real dy_deta  = dxyz_deta(1);
        Real dy_dzeta = dxyz_dzeta(1);

        Real dz_dxi   = dxyz_dxi(2);
        Real dz_deta  = dxyz_deta(2);
        Real dz_dzeta = dxyz_dzeta(2);

        phi(0) = (dx_dxi*vector_shape(0) + dx_deta*vector_shape(1) + dx_dzeta*vector_shape(2))/J;
        phi(1) = (dy_dxi*vector_shape(0) + dy_deta*vector_shape(1) + dy_dzeta*vector_shape(2))/J;
        phi(2) = (dz_dxi*vector_shape(0) + dz_deta*vector_shape(1) + dz_dzeta*vector_shape(2))/J;
...
RealGradient FEInterface::correct_vectorshape(const unsigned int dim,
                                              const FEType & fe_t,
                                              const Elem * elem,
                                              const unsigned int i,
                                              const Point & p)
{
  ...
  {
  case C_ZERO:
    return vectorshape(dim,fe_t,elem,i,p);

  case H_CURL:
    return vectorshape_hcurl(dim,fe_t,elem,i,p);

  case H_DIV:
    return vectorshape_hdiv(dim,fe_t,elem,i,p);

  default:
    libmesh_error_msg("Unsupported continuity! Currently only C_ZERO, H_CURL, and C_DIV is supported.");
  }
}
