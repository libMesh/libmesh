...
//-----------------------------------------------------------------------
// PetscAIJMatrix members


// Constructor
template <typename T>
PetscAIJMatrix<T>::PetscAIJMatrix(const Parallel::Communicator & comm_in) :
  PetscMatrix<T>(comm_in), _mat_type(AIJ)
{
}
...
// for destroying it
template <typename T>
PetscAIJMatrix<T>::PetscAIJMatrix(Mat mat_in,
                                  const Parallel::Communicator & comm_in) :
  PetscMatrix<T>(mat_in, comm_in)
{
  MatType mat_type;
...
// Destructor
template <typename T>
PetscAIJMatrix<T>::~PetscAIJMatrix() = default;



template <typename T>
void PetscAIJMatrix<T>::init (const numeric_index_type m_in,
...
template <typename T>
void PetscAIJMatrix<T>::init (const ParallelType)
{
...
template <typename T>
void PetscAIJMatrix<T>::update_preallocation_and_zero ()
{
  libmesh_not_implemented();
}

template <typename T>
void PetscAIJMatrix<T>::reset_preallocation()
{
...
template <typename T>
void PetscAIJMatrix<T>::zero_rows (std::vector<numeric_index_type> & rows, T diag_value)
{
  libmesh_assert (this->initialized());
...
template <typename T>
std::unique_ptr<SparseMatrix<T>> PetscAIJMatrix<T>::zero_clone () const
{
  ...
  // Call wrapping PetscAIJMatrix constructor, have it take over
  // ownership.
  auto ret = std::make_unique<PetscAIJMatrix<T>>(copy, this->comm());
...
template <typename T>
std::unique_ptr<SparseMatrix<T>> PetscAIJMatrix<T>::clone () const
{
  libmesh_error_msg_if(!this->closed(), "Matrix must be closed before it can be cloned!");
...
template <typename T>
Real PetscAIJMatrix<T>::l1_norm () const
{
  libmesh_assert (this->initialized());
...
template <typename T>
Real PetscAIJMatrix<T>::linfty_norm () const
{
  libmesh_assert (this->initialized());
...
template <typename T>
void PetscAIJMatrix<T>::print_matlab (const std::string & name) const
{
  libmesh_assert (this->initialized());
  ...
  if (!this->closed())
    {
      libmesh_deprecated();
      libmesh_warning("The matrix must be assembled before calling PetscAIJMatrix::print_matlab().\n"
                      "Please update your code, as this warning will become an error in a future release.");
      const_cast<PetscAIJMatrix<T> *>(this)->close();
    }
...
template <typename T>
void PetscAIJMatrix<T>::print_personal(std::ostream & os) const
{
  libmesh_assert (this->initialized());
  ...
  if (!this->closed())
    {
      libmesh_deprecated();
      libmesh_warning("The matrix must be assembled before calling PetscAIJMatrix::print_personal().\n"
                      "Please update your code, as this warning will become an error in a future release.");
      const_cast<PetscAIJMatrix<T> *>(this)->close();
    }
        ...
        if (this->processor_id() == 0)
          {
            ...
            libmesh_error_msg_if(fd == -1, "mkstemp failed in PetscAIJMatrix::print_personal()");
...
template <typename T>
void PetscAIJMatrix<T>::_petsc_viewer(const std::string & filename,
...
template <typename T>
void PetscAIJMatrix<T>::print_petsc_binary(const std::string & filename)
{
  libmesh_assert (this->initialized());
...
template <typename T>
void PetscAIJMatrix<T>::print_petsc_hdf5(const std::string & filename)
{
  libmesh_assert (this->initialized());
...
template <typename T>
void PetscAIJMatrix<T>::read_petsc_binary(const std::string & filename)
{
  LOG_SCOPE("read_petsc_binary()", "PetscAIJMatrix");
...
template <typename T>
void PetscAIJMatrix<T>::read_petsc_hdf5(const std::string & filename)
{
  LOG_SCOPE("read_petsc_hdf5()", "PetscAIJMatrix");
...
template <typename T>
void PetscAIJMatrix<T>::add_matrix(const DenseMatrix<T> & dm,
...
template <typename T>
void PetscAIJMatrix<T>::add_block_matrix(const DenseMatrix<T> & dm,
...
template <typename T>
void PetscAIJMatrix<T>::_get_submatrix(SparseMatrix<T> & submatrix,
                                       const std::vector<numeric_index_type> & rows,
                                       const std::vector<numeric_index_type> & cols,
                                       const bool reuse_submatrix) const
{
  if (!this->closed())
    {
      libmesh_deprecated();
      libmesh_warning("The matrix must be assembled before calling PetscAIJMatrix::create_submatrix().\n"
                      "Please update your code, as this warning will become an error in a future release.");
      const_cast<PetscAIJMatrix<T> *>(this)->close();
    }

  // Make sure the SparseMatrix passed in is really a PetscAIJMatrix
  PetscAIJMatrix<T> * petsc_submatrix = cast_ptr<PetscAIJMatrix<T> *>(&submatrix);
...
template <typename T>
void PetscAIJMatrix<T>::create_submatrix_nosort(SparseMatrix<T> & submatrix,
                                                const std::vector<numeric_index_type> & rows,
                                                const std::vector<numeric_index_type> & cols) const
{
  if (!this->closed())
    {
      libmesh_deprecated();
      libmesh_warning("The matrix must be assembled before calling PetscAIJMatrix::create_submatrix_nosort().\n"
                      "Please update your code, as this warning will become an error in a future release.");
      const_cast<PetscAIJMatrix<T> *>(this)->close();
    }
...
template <typename T>
void PetscAIJMatrix<T>::get_diagonal (NumericVector<T> & dest) const
{
  ...
  PetscErrorCode ierr =
    MatGetDiagonal(const_cast<PetscAIJMatrix<T> *>(this)->mat(),petsc_dest.vec()); LIBMESH_CHKERR(ierr);
}



template <typename T>
void PetscAIJMatrix<T>::get_transpose (SparseMatrix<T> & dest) const
{
  // Make sure the SparseMatrix passed in is really a PetscAIJMatrix
  PetscAIJMatrix<T> & petsc_dest = cast_ref<PetscAIJMatrix<T> &>(dest);
...
template <typename T>
void PetscAIJMatrix<T>::set (const numeric_index_type i,
...
template <typename T>
void PetscAIJMatrix<T>::add (const numeric_index_type i,
                             const numeric_index_type j,
                             const T value)
{
  libmesh_assert (this->initialized());
...
template <typename T>
void PetscAIJMatrix<T>::add (const T a_in, const SparseMatrix<T> & X_in)
{
  libmesh_assert (this->initialized());
  ...
  const PetscAIJMatrix<T> * X = cast_ptr<const PetscAIJMatrix<T> *> (&X_in);
...
template <typename T>
void PetscAIJMatrix<T>::matrix_matrix_mult (SparseMatrix<T> & X_in, SparseMatrix<T> & Y_out, bool reuse)
{
  libmesh_assert (this->initialized());
  ...
  const PetscAIJMatrix<T> * X = cast_ptr<const PetscAIJMatrix<T> *> (&X_in);
  PetscAIJMatrix<T> * Y = cast_ptr<PetscAIJMatrix<T> *> (&Y_out);
...
template <typename T>
void
PetscAIJMatrix<T>::add_sparse_matrix (const SparseMatrix<T> & spm,
                                      ...
                                      const T scalar)
{
  ...
  auto pscm = cast_ptr<const PetscAIJMatrix<T> *>(&spm);
...
template <typename T>
T PetscAIJMatrix<T>::operator () (const numeric_index_type i_in,
...
template <typename T>
void PetscAIJMatrix<T>::get_row (numeric_index_type i_in,
...
template <typename T>
PetscAIJMatrix<T> & PetscAIJMatrix<T>::operator= (const PetscAIJMatrix<T> & v)
{
  if (this->_mat)
    {
      PetscBool assembled;
...
template <typename T>
SparseMatrix<T> & PetscAIJMatrix<T>::operator= (const SparseMatrix<T> & v)
{
  *this = cast_ref<const PetscAIJMatrix<T> &>(v);
  return *this;
}

template <typename T>
void PetscAIJMatrix<T>::scale(const T scale)
{
  libmesh_assert(this->closed());
...
template class LIBMESH_EXPORT PetscAIJMatrix<Number>;
