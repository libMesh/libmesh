...
#ifndef LIBMESH_MESH_GENERATION_IMPL_H
#define LIBMESH_MESH_GENERATION_IMPL_H
...
public:
   ...
   */
  virtual void operator() (const PointTempl<RealType> & p,
                           const Real /*time*/,
   ...
   */
  virtual RealType operator() (const PointTempl<RealType> & /*p*/,
...
void MeshTools::Generation::build_cube(UnstructuredMeshTempl<RealType> & mesh,
                                       const unsigned int nx,
                                       const unsigned int ny,
    ...
    case 0:
      {
        ...
        mesh.add_point (PointTempl<RealType>(0, 0, 0), 0);
        ElemTempl<RealType> * elem = mesh.add_elem (new NodeElemTempl<RealType>);
    ...
    case 1:
      {
          ...
          case EDGE2:
            {
              for (unsigned int i=0; i<=nx; i++)
                mesh.add_point (PointTempl<RealType>(static_cast<Real>(i)/nx, 0, 0), node_id++);
          ...
          case EDGE3:
            {
              for (unsigned int i=0; i<=2*nx; i++)
                mesh.add_point (PointTempl<RealType>(static_cast<Real>(i)/(2*nx), 0, 0), node_id++);
              break;
            }
          ...
          case EDGE4:
            {
              for (unsigned int i=0; i<=3*nx; i++)
                mesh.add_point (PointTempl<RealType>(static_cast<Real>(i)/(3*nx), 0, 0), node_id++);
          ...
          case EDGE2:
            {
              for (unsigned int i=0; i<nx; i++)
                {
                  ElemTempl<RealType> * elem = new Edge2Templ<RealType>;
                  elem->set_id(i);
          ...
          case EDGE3:
            {
              for (unsigned int i=0; i<nx; i++)
                {
                  ElemTempl<RealType> * elem = new Edge3Templ<RealType>;
                  elem->set_id(i);
                  elem = mesh.add_elem (elem);
          ...
          case EDGE4:
            {
              for (unsigned int i=0; i<nx; i++)
                {
                  ElemTempl<RealType> * elem = new Edge4Templ<RealType>;
                  elem->set_id(i);
                  elem = mesh.add_elem (elem);
        ...
        if (gauss_lobatto_grid)
          {
            GaussLobattoRedistributionFunction<RealType> func(nx, xmin, xmax);
            MeshTools::Modification::redistribute(mesh, func);
          }
        else // !gauss_lobatto_grid
          {
            for (NodeTempl<RealType> * node : mesh.node_ptr_range())
          ...
          case TRI3:
            {
              for (unsigned int j=0; j<=ny; j++)
                for (unsigned int i=0; i<=nx; i++)
                  mesh.add_point (PointTempl<RealType>(static_cast<Real>(i)/static_cast<Real>(nx),
          ...
          case TRI6:
            {
              for (unsigned int j=0; j<=(2*ny); j++)
                for (unsigned int i=0; i<=(2*nx); i++)
                  mesh.add_point (PointTempl<RealType>(static_cast<Real>(i)/static_cast<Real>(2*nx),
          ...
          case QUAD4:
            {
              for (unsigned int j=0; j<ny; j++)
                for (unsigned int i=0; i<nx; i++)
                  {
                    ElemTempl<RealType> * elem = new Quad4Templ<RealType>;
                    elem->set_id(elem_id++);
                    elem = mesh.add_elem (elem);
          ...
          case TRI3:
            {
              for (unsigned int j=0; j<ny; j++)
                for (unsigned int i=0; i<nx; i++)
                  {
                    // Add first Tri3
                    ElemTempl<RealType> * elem = new Tri3Templ<RealType>;
                    elem->set_id(elem_id++);
                    elem = mesh.add_elem (elem);
          ...
          case QUAD9:
            {
              for (unsigned int j=0; j<(2*ny); j += 2)
                for (unsigned int i=0; i<(2*nx); i += 2)
                  {
                    ElemTempl<RealType> * elem = (type == QUAD8) ?
                      static_cast<ElemTempl<RealType> *>(new Quad8Templ<RealType>) :
                      static_cast<ElemTempl<RealType> *>(new Quad9Templ<RealType>);
                    elem->set_id(elem_id++);
                    elem = mesh.add_elem (elem);
          ...
          case TRI6:
            {
              for (unsigned int j=0; j<(2*ny); j += 2)
                for (unsigned int i=0; i<(2*nx); i += 2)
                  {
                    // Add first Tri6
                    ElemTempl<RealType> * elem = new Tri6Templ<RealType>;
                    elem->set_id(elem_id++);
                    elem = mesh.add_elem (elem);
    ...
    case 3:
      {
          ...
          case HEX8:
            {
              for (unsigned int k=0; k<nz; k++)
                for (unsigned int j=0; j<ny; j++)
                  for (unsigned int i=0; i<nx; i++)
                    {
                      ElemTempl<RealType> * elem = new Hex8Templ<RealType>;
                      elem->set_id(elem_id++);
                      elem = mesh.add_elem (elem);
              ...
              for (unsigned int k=0; k<nz; k++)
                for (unsigned int j=0; j<ny; j++)
                  for (unsigned int i=0; i<nx; i++)
                    {
                      // First Prism
                      ElemTempl<RealType> * elem = new Prism6Templ<RealType>;
                      elem->set_id(elem_id++);
                      elem = mesh.add_elem (elem);
          ...
          case PYRAMID14:
            {
              for (unsigned int k=0; k<(2*nz); k += 2)
                for (unsigned int j=0; j<(2*ny); j += 2)
                  for (unsigned int i=0; i<(2*nx); i += 2)
                    {
                      ElemTempl<RealType> * elem = (type == HEX20) ?
                        static_cast<ElemTempl<RealType> *>(new Hex20Templ<RealType>) :
                        static_cast<ElemTempl<RealType> *>(new Hex27Templ<RealType>);
                      elem->set_id(elem_id++);
                      elem = mesh.add_elem (elem);
              ...
              for (unsigned int k=0; k<(2*nz); k += 2)
                for (unsigned int j=0; j<(2*ny); j += 2)
                  for (unsigned int i=0; i<(2*nx); i += 2)
                    {
                      // First Prism
                      ElemTempl<RealType> * elem = (type == PRISM15) ?
                        static_cast<ElemTempl<RealType> *>(new Prism15Templ<RealType>) :
                        static_cast<ElemTempl<RealType> *>(new Prism18Templ<RealType>);
                      elem->set_id(elem_id++);
                      elem = mesh.add_elem (elem);
        ...
        if ((type == TET4) ||
            ...
            (type == PYRAMID14))
          {
            ...
            std::vector<ElemTempl<RealType> *> new_elements;
            ...
            for (auto & base_hex : mesh.element_ptr_range())
              {
                ...
                NodeTempl<RealType> * apex_node = base_hex->node_ptr(26);
                ...
                for (auto s : base_hex->side_index_range())
                  {
                    ...
                    std::unique_ptr<ElemTempl<RealType>> side = base_hex->build_side_ptr(s);
                    ...
                    if ((type == TET4) || (type == TET10))
                      {
                        ...
                        for (unsigned int sub_tet=0; sub_tet<4; ++sub_tet)
                          {
                            new_elements.push_back( new Tet4Templ<RealType> );
                            ElemTempl<RealType> * sub_elem = new_elements.back();
...
template <typename RealType>
void MeshTools::Generation::build_point (UnstructuredMeshTempl<RealType> & mesh,
                                         const ElemType type,
                                         const bool gauss_lobatto_grid)
{
...
template <typename RealType>
void MeshTools::Generation::build_line (UnstructuredMeshTempl<RealType> & mesh,
                                        const unsigned int nx,
                                        const Real xmin, const Real xmax,
                                        const ElemType type,
                                        const bool gauss_lobatto_grid)
...
template <typename RealType>
void MeshTools::Generation::build_square (UnstructuredMeshTempl<RealType> & mesh,
                                          const unsigned int nx,
                                          const unsigned int ny,
                                          const Real xmin, const Real xmax,
                                          const Real ymin, const Real ymax,
...
template <typename RealType>
void MeshTools::Generation::build_sphere (UnstructuredMeshTempl<RealType> &,
                                          const Real,
                                          const unsigned int,
                                          const ElemType,
                                          const unsigned int,
...
template <typename RealType>
void MeshTools::Generation::build_sphere (UnstructuredMeshTempl<RealType> & mesh,
                                          const Real rad,
                                          ...
                                          const bool flat)
{
  ...
  const PointTempl<RealType> cent;
        ...
        if (flat)
          {
            ...
            std::vector<NodeTempl<RealType> *> nodes(8);

            // PointTempl<RealType> 0
            nodes[0] = mesh.add_point (PointTempl<RealType>(-rad_2,-rad_2, 0.), node_id++);

            // PointTempl<RealType> 1
            nodes[1] = mesh.add_point (PointTempl<RealType>( rad_2,-rad_2, 0.), node_id++);

            // PointTempl<RealType> 2
            nodes[2] = mesh.add_point (PointTempl<RealType>( rad_2, rad_2, 0.), node_id++);

            // PointTempl<RealType> 3
            nodes[3] = mesh.add_point (PointTempl<RealType>(-rad_2, rad_2, 0.), node_id++);

            // PointTempl<RealType> 4
            nodes[4] = mesh.add_point (PointTempl<RealType>(-rad_sqrt_2,-rad_sqrt_2, 0.), node_id++);

            // PointTempl<RealType> 5
            nodes[5] = mesh.add_point (PointTempl<RealType>( rad_sqrt_2,-rad_sqrt_2, 0.), node_id++);

            // PointTempl<RealType> 6
            nodes[6] = mesh.add_point (PointTempl<RealType>( rad_sqrt_2, rad_sqrt_2, 0.), node_id++);

            // PointTempl<RealType> 7
            nodes[7] = mesh.add_point (PointTempl<RealType>(-rad_sqrt_2, rad_sqrt_2, 0.), node_id++);

            // Build the elements & set node pointers

            // Element 0
            {
              ElemTempl<RealType> * elem0 = mesh.add_elem (new Quad4Templ<RealType>);
            ...
            // Element 1
            {
              ElemTempl<RealType> * elem1 = mesh.add_elem (new Quad4Templ<RealType>);
            ...
            // Element 2
            {
              ElemTempl<RealType> * elem2 = mesh.add_elem (new Quad4Templ<RealType>);
            ...
            // Element 3
            {
              ElemTempl<RealType> * elem3 = mesh.add_elem (new Quad4Templ<RealType>);
            ...
            // Element 4
            {
              ElemTempl<RealType> * elem4 = mesh.add_elem (new Quad4Templ<RealType>);
        ...
        else
          {
            ...
            mesh.add_point (PointTempl<RealType>(-s,  t,  0), node_id++);
            mesh.add_point (PointTempl<RealType>( s,  t,  0), node_id++);
            mesh.add_point (PointTempl<RealType>(-s, -t,  0), node_id++);
            mesh.add_point (PointTempl<RealType>( s, -t,  0), node_id++);

            mesh.add_point (PointTempl<RealType>( 0, -s,  t), node_id++);
            mesh.add_point (PointTempl<RealType>( 0,  s,  t), node_id++);
            mesh.add_point (PointTempl<RealType>( 0, -s, -t), node_id++);
            mesh.add_point (PointTempl<RealType>( 0,  s, -t), node_id++);

            mesh.add_point (PointTempl<RealType>( t,  0, -s), node_id++);
            mesh.add_point (PointTempl<RealType>( t,  0,  s), node_id++);
            mesh.add_point (PointTempl<RealType>(-t,  0, -s), node_id++);
            mesh.add_point (PointTempl<RealType>(-t,  0,  s), node_id++);
    ...
    case 3:
      {
        ...
        // (Temporary) convenient storage for node pointers
        std::vector<NodeTempl<RealType> *> nodes(16);
        ...
        // PointTempl<RealType>s 0-7 are the initial HEX8
        nodes[0] = mesh.add_point (PointTempl<RealType>(-r_small,-r_small, -r_small), node_id++);
        nodes[1] = mesh.add_point (PointTempl<RealType>( r_small,-r_small, -r_small), node_id++);
        nodes[2] = mesh.add_point (PointTempl<RealType>( r_small, r_small, -r_small), node_id++);
        nodes[3] = mesh.add_point (PointTempl<RealType>(-r_small, r_small, -r_small), node_id++);
        nodes[4] = mesh.add_point (PointTempl<RealType>(-r_small,-r_small,  r_small), node_id++);
        nodes[5] = mesh.add_point (PointTempl<RealType>( r_small,-r_small,  r_small), node_id++);
        nodes[6] = mesh.add_point (PointTempl<RealType>( r_small, r_small,  r_small), node_id++);
        nodes[7] = mesh.add_point (PointTempl<RealType>(-r_small, r_small,  r_small), node_id++);

        //  PointTempl<RealType>s 8-15 are for the outer hexes, we number them in the same way
        nodes[8]  = mesh.add_point (PointTempl<RealType>(-r_med,-r_med, -r_med), node_id++);
        nodes[9]  = mesh.add_point (PointTempl<RealType>( r_med,-r_med, -r_med), node_id++);
        nodes[10] = mesh.add_point (PointTempl<RealType>( r_med, r_med, -r_med), node_id++);
        nodes[11] = mesh.add_point (PointTempl<RealType>(-r_med, r_med, -r_med), node_id++);
        nodes[12] = mesh.add_point (PointTempl<RealType>(-r_med,-r_med,  r_med), node_id++);
        nodes[13] = mesh.add_point (PointTempl<RealType>( r_med,-r_med,  r_med), node_id++);
        nodes[14] = mesh.add_point (PointTempl<RealType>( r_med, r_med,  r_med), node_id++);
        nodes[15] = mesh.add_point (PointTempl<RealType>(-r_med, r_med,  r_med), node_id++);

        // Now create the elements and add them to the mesh
        // Element 0 - center element
        {
          ElemTempl<RealType> * elem0 = mesh.add_elem (new Hex8Templ<RealType>);
          elem0->set_node(0) = nodes[0];
          elem0->set_node(1) = nodes[1];
          elem0->set_node(2) = nodes[2];
          elem0->set_node(3) = nodes[3];
        ...
        // Element 1 - "bottom"
        {
          ElemTempl<RealType> * elem1 = mesh.add_elem (new Hex8Templ<RealType>);
        ...
        // Element 2 - "front"
        {
          ElemTempl<RealType> * elem2 = mesh.add_elem (new Hex8Templ<RealType>);
        ...
        // Element 3 - "right"
        {
          ElemTempl<RealType> * elem3 = mesh.add_elem (new Hex8Templ<RealType>);
          elem3->set_node(0) = nodes[1];
        ...
        // Element 4 - "back"
        {
          ElemTempl<RealType> * elem4 = mesh.add_elem (new Hex8Templ<RealType>);
          elem4->set_node(0) = nodes[3];
          elem4->set_node(1) = nodes[2];
        ...
        // Element 5 - "left"
        {
          ElemTempl<RealType> * elem5 = mesh.add_elem (new Hex8Templ<RealType>);
        ...
        // Element 6 - "top"
        {
          ElemTempl<RealType> * elem6 = mesh.add_elem (new Hex8Templ<RealType>);
      ...
      for (const auto & elem : mesh.active_element_ptr_range())
        for (auto s : elem->side_index_range())
          if (elem->neighbor_ptr(s) == nullptr || (mesh.mesh_dimension() == 2 && !flat))
            {
              std::unique_ptr<ElemTempl<RealType>> side(elem->build_side_ptr(s));
  ...
  if (ElemTempl<RealType>::build(type)->default_order() != FIRST)
    {
...
template <typename RealType>
void MeshTools::Generation::build_extrusion (UnstructuredMeshTempl<RealType> & mesh,
                                             const MeshBaseTempl<RealType> & cross_section,
                                             const unsigned int nz,
                                             RealVectorValue extrusion_vector,
                                             QueryElemSubdomainIDBaseTempl<RealType> * elem_subdomain)
{
  ...
  for (const auto & elem : cross_section.element_ptr_range())
    {
      ...
      for (unsigned int k=0; k != nz; ++k)
        {
          ElemTempl<RealType> * new_elem;
          switch (etype)
            {
            case EDGE2:
              {
                new_elem = new Quad4Templ<RealType>;
                ...
                if (elem->neighbor_ptr(0) == RemoteElem::get_instance())
                  new_elem->set_neighbor(3, const_cast<RemoteElemTempl<RealType> *>(RemoteElem::get_instance()));
                if (elem->neighbor_ptr(1) == RemoteElem::get_instance())
                  new_elem->set_neighbor(1, const_cast<RemoteElemTempl<RealType> *>(RemoteElem::get_instance()));

                break;
              }
            case EDGE3:
              {
                new_elem = new Quad9Templ<RealType>;
            ...
            case TRI3:
              {
                new_elem = new Prism6Templ<RealType>;
                new_elem->set_node(0) = mesh.node_ptr(elem->node_ptr(0)->id() + (k * orig_nodes));
                new_elem->set_node(1) = mesh.node_ptr(elem->node_ptr(1)->id() + (k * orig_nodes));
                ...
                if (elem->neighbor_ptr(0) == RemoteElem::get_instance())
                  new_elem->set_neighbor(1, const_cast<RemoteElemTempl<RealType> *>(RemoteElem::get_instance()));
                if (elem->neighbor_ptr(1) == RemoteElem::get_instance())
                  new_elem->set_neighbor(2, const_cast<RemoteElemTempl<RealType> *>(RemoteElem::get_instance()));
                if (elem->neighbor_ptr(2) == RemoteElem::get_instance())
                  new_elem->set_neighbor(3, const_cast<RemoteElemTempl<RealType> *>(RemoteElem::get_instance()));
            ...
            case TRI6:
              {
                new_elem = new Prism18Templ<RealType>;
                new_elem->set_node(0) = mesh.node_ptr(elem->node_ptr(0)->id() + (2*k * orig_nodes));
                new_elem->set_node(1) = mesh.node_ptr(elem->node_ptr(1)->id() + (2*k * orig_nodes));
            ...
            case QUAD4:
              {
                new_elem = new Hex8Templ<RealType>;
                new_elem->set_node(0) = mesh.node_ptr(elem->node_ptr(0)->id() + (k * orig_nodes));
                new_elem->set_node(1) = mesh.node_ptr(elem->node_ptr(1)->id() + (k * orig_nodes));
                new_elem->set_node(2) = mesh.node_ptr(elem->node_ptr(2)->id() + (k * orig_nodes));
                ...
                if (elem->neighbor_ptr(2) == RemoteElem::get_instance())
                  new_elem->set_neighbor(3, const_cast<RemoteElemTempl<RealType> *>(RemoteElem::get_instance()));
                if (elem->neighbor_ptr(3) == RemoteElem::get_instance())
                  new_elem->set_neighbor(4, const_cast<RemoteElemTempl<RealType> *>(RemoteElem::get_instance()));

                break;
              }
            case QUAD9:
              {
                new_elem = new Hex27Templ<RealType>;
                new_elem->set_node(0) = mesh.node_ptr(elem->node_ptr(0)->id() + (2*k * orig_nodes));
                new_elem->set_node(1) = mesh.node_ptr(elem->node_ptr(1)->id() + (2*k * orig_nodes));
                new_elem->set_node(2) = mesh.node_ptr(elem->node_ptr(2)->id() + (2*k * orig_nodes));
...
template <typename RealType>
void MeshTools::Generation::build_delaunay_square(UnstructuredMeshTempl<RealType> & mesh,
                                                  ...
                                                  const std::vector<TriangleInterface::Hole*> * holes)
{
  ...
  for (unsigned int p=0; p<=nx; ++p)
    mesh.add_point(PointTempl<RealType>(xmin + p*delta_x, ymin));

  // Right side
  for (unsigned int p=1; p<ny; ++p)
    mesh.add_point(PointTempl<RealType>(xmax, ymin + p*delta_y));

  // Top
  for (unsigned int p=0; p<=nx; ++p)
    mesh.add_point(PointTempl<RealType>(xmax - p*delta_x, ymax));

  // Left side
  for (unsigned int p=1; p<ny; ++p)
    mesh.add_point(PointTempl<RealType>(xmin,  ymax - p*delta_y));
  ...
  for (auto & elem : mesh.element_ptr_range())
    for (auto s : elem->side_index_range())
      if (elem->neighbor_ptr(s) == nullptr)
        {
          std::unique_ptr<const ElemTempl<RealType>> side (elem->build_side_ptr(s));
          ...
          // sides.
          PointTempl<RealType> side_midpoint= 0.5f*( side->point(0) + side->point(1) );
...
#define INSTANTIATE_MESH_GENERATION_METHODS_ZERO(RealType)                                         \
  template void MeshTools::Generation::build_cube(UnstructuredMeshTempl<RealType> & mesh,          \
                                                  const unsigned int nx,                           \
                                                  const unsigned int ny,                           \
                                                  const unsigned int nz,                           \
                                                  const Real xmin,                                 \
                                                  ...
                                                  const bool gauss_lobatto_grid);                  \
                                                                                                   \
  template void MeshTools::Generation::build_point(                                                \
      UnstructuredMeshTempl<Real> & mesh, const ElemType type, const bool gauss_lobatto_grid);     \
                                                                                                   \
  template void MeshTools::Generation::build_line(UnstructuredMeshTempl<RealType> & mesh,          \
                                                  const unsigned int nx,                           \
                                                  const Real xmin,                                 \
                                                  const Real xmax,                                 \
                                                  const ElemType type,                             \
                                                  const bool gauss_lobatto_grid);                  \
                                                                                                   \
  template void MeshTools::Generation::build_square(UnstructuredMeshTempl<RealType> & mesh,        \
                                                    const unsigned int nx,                         \
                                                    const unsigned int ny,                         \
                                                    const Real xmin,                               \
                                                    const Real xmax,                               \
                                                    ...
                                                    const bool gauss_lobatto_grid);                \
                                                                                                   \
  template void MeshTools::Generation::build_extrusion(                                            \
      UnstructuredMeshTempl<RealType> & mesh,                                                      \
      const MeshBaseTempl<RealType> & cross_section,                                               \
      const unsigned int nz,                                                                       \
      RealVectorValue extrusion_vector,                                                            \
      QueryElemSubdomainIDBaseTempl<RealType> * elem_subdomain)
...
#ifndef LIBMESH_ENABLE_AMR
#define INSTANTIATE_MESH_GENERATION_METHODS_ONE(RealType)                                          \
  INSTANTIATE_MESH_GENERATION_METHODS_ZERO(RealType);                                              \
  template void MeshTools::Generation::build_sphere(UnstructuredMeshTempl<RealType> &,             \
                                                    const Real,                                    \
                                                    const unsigned int,                            \
                                                    const ElemType,                                \
                                                    const unsigned int,                            \
...
#define INSTANTIATE_MESH_GENERATION_METHODS_ONE(RealType)                                          \
  INSTANTIATE_MESH_GENERATION_METHODS_ZERO(RealType);                                              \
  template void MeshTools::Generation::build_sphere(UnstructuredMeshTempl<RealType> & mesh,        \
                                                    const Real rad,                                \
                                                    const unsigned int nr,                         \
                                                    const ElemType type,                           \
                                                    const unsigned int n_smooth,                   \
...
#if defined(LIBMESH_HAVE_TRIANGLE) && LIBMESH_DIM > 1
#define INSTANTIATE_MESH_GENERATION_METHODS(RealType)                                              \
  INSTANTIATE_MESH_GENERATION_METHODS_ONE(RealType);                                               \
  template void MeshTools::Generation::build_delaunay_square(                                      \
      UnstructuredMeshTempl<RealType> & mesh,                                                      \
      const unsigned int nx,                                                                       \
      const unsigned int ny,                                                                       \
      const Real xmin,                                                                             \
...
#endif // LIBMESH_MESH_GENERATION_IMPL_H
