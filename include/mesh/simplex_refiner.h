...
private:
  ...
  // done to this edge
  typedef
    std::vector<std::tuple<dof_id_type, dof_id_type, dof_id_type>>
    refinement_datum;
  ...
  /**
   * Keep track of elements added so we can answer queries about them
   * from other processors.  If the element originally with id i is
   * split, its replacements e1 and e2 will be in
   * added_elements[i][ei.vertex_average()]
   */
