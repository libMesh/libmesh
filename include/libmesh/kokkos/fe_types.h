// Kokkos FE type enumerations and conversion helpers.
//
// Defines the runtime enumerations (FEElemTopology, FEFamily, FEElemClass),
// the FEShapeKey aggregate, and host-side conversion functions from libMesh
// enumerations.  These types are used by both host-side assembly setup and,
// where annotated, device-side evaluation.
...
// Integer keys used to dispatch to the right FEEvaluator specialization at
// runtime (CPU only). Values are contiguous so they can serve directly as
// array indices.
...
/// Throws for element types not supported by the native path.
FEElemTopology toKokkosTopology(libMesh::ElemType t);

/// Convert a libMesh FEFamily to the Kokkos runtime family enum.
/// Returns FEFamily::UNKNOWN for families not supported by the native path.
