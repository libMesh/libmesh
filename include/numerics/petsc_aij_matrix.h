...
#ifndef LIBMESH_PETSC_AIJ_MATRIX_H
#define LIBMESH_PETSC_AIJ_MATRIX_H
 ...
 */
template <typename T>
class PetscAIJMatrix final : public PetscMatrix<T>
{
public:
  /**
   ...
   */
  explicit
  PetscAIJMatrix (const Parallel::Communicator & comm_in);

  /**
   * Constructor.  Creates a PetscAIJMatrix assuming you already have a
   * valid Mat object.  In this case, m is NOT destroyed by the
   * PetscAIJMatrix destructor when this object goes out of scope.  This
   * allows ownership of m to remain with the original creator, and to
   * simply provide additional functionality with the PetscAIJMatrix.
   */
  explicit
  PetscAIJMatrix (Mat m,
   ...
   */
  PetscAIJMatrix (PetscAIJMatrix &&) = delete;
  PetscAIJMatrix (const PetscAIJMatrix &) = delete;
  PetscAIJMatrix & operator= (PetscAIJMatrix &&) = delete;
  virtual ~PetscAIJMatrix ();

  PetscAIJMatrix & operator= (const PetscAIJMatrix &);
...
#endif // LIBMESH_PETSC_AIJ_MATRIX_H
