...
#ifndef LIBMESH_PARMETIS_PARTITIONER_IMPL_H
#define LIBMESH_PARMETIS_PARTITIONER_IMPL_H
...
ParmetisPartitionerTempl<RealType>::ParmetisPartitionerTempl()
#ifdef LIBMESH_HAVE_PARMETIS
...
template <typename RealType>
ParmetisPartitionerTempl<RealType>::ParmetisPartitionerTempl (const ParmetisPartitioner & other)
  : Partitioner(other)
#ifdef LIBMESH_HAVE_PARMETIS
...
template <typename RealType>
ParmetisPartitionerTempl<RealType>::~ParmetisPartitionerTempl() = default;



template <typename RealType>
void ParmetisPartitionerTempl<RealType>::_do_partition (MeshBase & mesh,
...
template <typename RealType>
void ParmetisPartitionerTempl<RealType>::_do_repartition (MeshBase & mesh,
                                           const unsigned int n_sbdmns)
{
  ...
  // per partition or when it can't reach elements from each other.
  {
    ...
    for (const auto & nelem : this->_n_active_elem_on_proc)
...
template <typename RealType>
void ParmetisPartitionerTempl<RealType>::initialize (const MeshBase & mesh,
                                      const unsigned int n_sbdmns)
{
  ...
  this->_find_global_index_by_pid_map(mesh);
  ...
  for (auto pid : IntRange<processor_id_type>(0, mesh.n_processors()))
    {
      _pmetis->vtxdist[pid+1] = _pmetis->vtxdist[pid] + this->_n_active_elem_on_proc[pid];
      n_active_elem += this->_n_active_elem_on_proc[pid];
    }
...

  {
    ...
    {
      typename MeshBase::const_element_iterator       it  = mesh.active_elements_begin();
      const typename MeshBase::const_element_iterator end = mesh.active_elements_end();
    ...
    libmesh_assert_less_equal (this->_global_index_by_pid_map.size(), n_active_elem);
    ...
    if (global_index_map.size() != this->_global_index_by_pid_map.size())
  ...
  // defined by a simple mapping of the global indices we just found.
  {
    ...
    for (const auto & elem : mesh.active_local_element_ptr_range())
      {
        libmesh_assert (this->_global_index_by_pid_map.count(elem->id()));
        const dof_id_type global_index_by_pid =
          this->_global_index_by_pid_map[elem->id()];
...
template <typename RealType>
void ParmetisPartitionerTempl<RealType>::build_graph (const MeshBase & mesh)
{
  ...
  for (auto & graph_row : this->_dual_graph)
    {
...
#endif // LIBMESH_PARMETIS_PARTITIONER_IMPL_H
