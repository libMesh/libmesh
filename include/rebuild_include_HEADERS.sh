...
{
echo "# These are headers we need to compile the libMesh library but should"
echo "# not be installed.  These header files may have implementation"
echo "# details which are not required for the public interface"
} >> include_HEADERS
...
headers=$(find base enums error_estimation fe geom ghosting gpu mesh numerics parallel partitioning timpi_shims physics quadrature reduced_basis solution_transfer solvers systems utils -name "*.h" -o -name "*specializations" -type f | LC_COLLATE=POSIX sort)
headers="libmesh_config.h $headers"

{
echo "# These are the headers we actually want to install and make available"
echo "# to a user.  Consider these the public interface of libMesh."
echo "# will get installed into '\$(prefix)/include/libmesh'"
} >> include_HEADERS
