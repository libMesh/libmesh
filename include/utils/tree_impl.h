...
Tree<N,RealType>::Tree (const MeshBase & m,
  ...
  build_type(bt)
{
  ...
  root.set_bounding_box (MeshTools::create_bounding_box(this->mesh));

  if (build_type == Trees::NODES)
    {
      // Add all the nodes to the root node.  It will
      // automagically build the tree for us.
      for (const auto & node : this->mesh.node_ptr_range())
        {
#ifndef NDEBUG
      ...
      MeshTools::build_nodes_to_elem_map (this->mesh, nodes_to_elem);
  ...
  else if (build_type == Trees::ELEMENTS)
    {
      ...
      for (const auto & elem : this->mesh.active_element_ptr_range())
        {
#ifndef NDEBUG
  ...
  else if (build_type == Trees::LOCAL_ELEMENTS)
    {
      ...
      // automatically build the tree for us.
      for (const auto & elem : this->mesh.active_local_element_ptr_range())
        {
#ifndef NDEBUG
          bool elem_was_inserted =
#endif
...
template <unsigned int N, typename RealType>
Tree<N,RealType>::Tree (const Tree<N> & other_tree) :
  TreeBase   (other_tree),
...
template <unsigned int N, typename RealType>
void Tree<N,RealType>::print_nodes(std::ostream & my_out) const
{
...
template <unsigned int N, typename RealType>
void Tree<N,RealType>::print_elements(std::ostream & my_out) const
{
...
Tree<N,RealType>::find_element (const Point & p,
...
template <unsigned int N, typename RealType>
void
Tree<N,RealType>::find_elements (const Point & p,
...
template <unsigned int N, typename RealType>
const ElemTempl<RealType> *
Tree<N,RealType>::operator() (const Point & p,
                     const std::set<subdomain_id_type> * allowed_subdomains,
                     Real relative_tol) const
{
