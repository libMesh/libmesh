...
/**
 * The StatisticsVector class is derived from the std::vector<> and
 ...
 * anticipate any problems with deriving from an stl container which
 * lacks a virtual destructor in this case.
 *
 * Where manipulation of the data set was necessary (for example
 * sorting) two versions of member functions have been implemented.
 * The non-const versions perform sorting directly in the data set,
 * invalidating pointers and changing the entries.  const versions of
 * the same functions are generally available, and will be
 * automatically invoked on const StatisticsVector objects.  A
 * draw-back to the const versions is that they simply make a copy of
 * the original object and therefore double the original memory
 * requirement for the data set.
 *
 * Most of the actual code was copied or adapted from the GNU
 * Scientific Library (GSL). More precisely, the recursion relations
 * for computing the mean were implemented in order to avoid possible
...
public:
  ...
  /**
   * \returns The mean value of the data set using a recurrence
   * relation.
   *
  ...
  /**
   * \returns The variance of the data set where the \p mean is
   * provided.
   *
  ...
  /**
   * \returns The standard deviation of the data set, which is simply
  ...
  /**
   * \returns Computes the standard deviation of the data set, which
   * is simply the square-root of the variance.
   *
   * This method can be used for efficiency when the \p mean has
  ...
  /**
   * \returns A histogram with n_bins bins for the data set.
   *
   * For simplicity, the bins are assumed to be of uniform size.  Upon
   * return, the bin_members vector will contain unsigned integers
   * which give the number of members in each bin.  WARNING: This
   * non-const function sorts the vector, changing its order.  Source:
  ...
  /**
   * \returns A vector of dof_id_types which corresponds
  ...
  /**
   * \returns A vector of dof_id_types which corresponds to the
   * indices of every member of the data set above the cutoff value
   * cut.
   *
   * I chose not to combine these two functions since the interface is
   * cleaner with one passed parameter instead of two.
   */
