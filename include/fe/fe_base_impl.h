...
FEGenericBase<OutputType,RealType>::FEGenericBase(const unsigned int d,
  ...
  _fe_trans( FETransformationBase<OutputType,RealType>::build(fet) ),
  phi(),
  dphi(),
  curl_phi(),
  div_phi(),
                                                          ...
                                                          const bool use_old_dof_indices)
{
  ...
  std::vector<PointTempl<Real>> coarse_qpoints;

  // The values of the shape functions at the quadrature
  // points
  const std::vector<std::vector<OutputType>> & phi_values =
    fe->get_phi();
  const std::vector<std::vector<OutputType>> & phi_coarse =
    fe_coarse->get_phi();

  typedef typename TensorTools::IncrementRank<OutputType>::type MyGradient;
  typedef typename TensorTools::MakeNumber<OutputType>::type MyNumber;
  typedef typename TensorTools::IncrementRank<MyNumber>::type MyNumberGradient;

  // The gradients of the shape functions at the quadrature
  // points on the child element.
  const std::vector<std::vector<MyGradient>> * dphi_values =
    nullptr;
  const std::vector<std::vector<MyGradient>> * dphi_coarse =
    nullptr;
  ...
  if (cont == C_ONE)
    {
      const std::vector<std::vector<MyGradient>> &
  ...
  // child element
  const std::vector<PointTempl<Real>> & xyz_values =
    fe->get_xyz();
        ...
        for (unsigned int c=0; c != n_children; ++c)
          {
            if (!elem->is_child_on_edge(c,e))
              continue;
            const ElemTempl<Real> * child = elem->child_ptr(c);
            ...
            FEMapTempl<Real>::inverse_map (dim, elem, xyz_values,
                                coarse_qpoints);
            ...
            for (unsigned int qp=0; qp<n_qp; qp++)
              {
                ...
                MyNumber fineval = libMesh::zero;
                // solution grad at the quadrature point
                MyNumberGradient finegrad;
  ...
  for (auto & child : elem->child_ref_range())
    {
      ...
      FEMapTempl<Real>::inverse_map (dim, elem, xyz_values, coarse_qpoints);
                                                               ...
                                                               const ElemTempl<Real> * elem)
{
  ...
  std::vector<PointTempl<Real>> neigh_qface;

  const std::vector<Real> & JxW = my_fe->get_JxW();
  const std::vector<PointTempl<Real>> & q_point = my_fe->get_xyz();
  const std::vector<std::vector<OutputType>> & phi = my_fe->get_phi();
  const std::vector<std::vector<OutputType>> & neigh_phi =
    neigh_fe->get_phi();
  const std::vector<PointTempl<Real>> * face_normals = nullptr;

  typedef typename TensorTools::IncrementRank<OutputType>::type MyGradient;
  typedef MyGradient NeighborGradient;

  const std::vector<std::vector<MyGradient>> * dphi = nullptr;
  const std::vector<std::vector<NeighborGradient>> * neigh_dphi = nullptr;
  ...
  if (cont != C_ZERO)
    {
      const std::vector<PointTempl<Real>> & ref_face_normals =
        my_fe->get_normals();
      face_normals = &ref_face_normals;
      const std::vector<std::vector<MyGradient>> & ref_dphi =
        my_fe->get_dphi();
      dphi = &ref_dphi;
      const std::vector<std::vector<NeighborGradient>> & ref_neigh_dphi =
        neigh_fe->get_dphi();
  ...
  for (auto s : elem->side_index_range())
    {
      ...
      const ElemTempl<Real> * neigh = elem->neighbor_ptr(s);
      ...
      if (neigh->level() < elem->level())
        {
          ...
          FEMapTempl<Real>::inverse_map (Dim, neigh, q_point, neigh_qface);
          ...
          if (periodic && periodic->is_my_variable(variable_number))
            {
              ...
              // Get pointers to the element's neighbor.
              const ElemTempl<Real> * neigh = boundaries.neighbor(boundary_id, *point_locator, elem, s);
              ...
              if (neigh->level() <= elem->level())
                {
                  ...
                  const unsigned int old_elem_level = elem->p_level();
                  if (old_elem_level != min_p_level)
                    (const_cast<ElemTempl<Real> *>(elem))->hack_p_level(min_p_level);
                  const unsigned int old_neigh_level = neigh->p_level();
                  if (old_neigh_level != min_p_level)
                    (const_cast<ElemTempl<Real> *>(neigh))->hack_p_level(min_p_level);
#endif // #ifdef LIBMESH_ENABLE_AMR
                  ...
                  // neighbor's boundary
                  std::vector<PointTempl<Real>> neigh_point(q_point.size());
                  for (auto i : index_range(neigh_point))
                    neigh_point[i] = periodic->get_corresponding_pos(q_point[i]);

                  FEMapTempl<Real>::inverse_map (Dim, neigh, neigh_point,
                                      neigh_qface);
                  ...
                  if (elem->p_level() != old_elem_level)
                    (const_cast<ElemTempl<Real> *>(elem))->hack_p_level(old_elem_level);
                  if (neigh->p_level() != old_neigh_level)
                    (const_cast<ElemTempl<Real> *>(neigh))->hack_p_level(old_neigh_level);
#endif // #ifdef LIBMESH_ENABLE_AMR
                  ...
                  for (auto n : elem->node_index_range())
                    {
                      ...
                      const NodeTempl<Real> & my_node = elem->node_ref(n);
                      ...
                      if (elem->is_vertex(n))
                        {
                          ...
                          const ElemTempl<Real> * primary_elem = nullptr;
                          const ElemTempl<Real> * main_neigh = nullptr;
                          PointTempl<Real> main_pt = my_node,
                            primary_pt = my_node;
                          ...
                          for (const auto & new_boundary_id : point_bcids)
                            {
                              ...
                              const PointTempl<Real> neigh_pt =
                              ...
                              const ElemTempl<Real> * primary_neigh =
                          ...
                          for (const auto & new_boundary_id : edge_bcids)
                            {
                              ...
                              PointTempl<Real> neigh_pt1 = new_periodic->get_corresponding_pos(*e1),
                              ...
                              const ElemTempl<Real> * primary_neigh = primary_boundary_edge_neighbor
