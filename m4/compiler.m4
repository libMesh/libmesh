...
[
  ...
  dnl If we already have CC either from --with-cc or from the environment, then there's no sense going
  ...
  AS_IF([test -z "$CC" && test "$enablempi" != no],
        [
  ...
  dnl See whether we are using PETSC_CC. Unfortunately PETSC_CC may
  dnl be prefixed with a PATH, and its not straightforward to strip it off.
  dnl If CC is not set AC_PROG_CC will call AC_CHECK_TOOLS which will prefix
  dnl every argument in CC_TRY_LIST with values in $PATH, so we will
  dnl not find something like PATH/PETSC_PREFIX/mpicxx. The solution
  dnl then is just to set CC to PETSC_CC so that AC_CHECK_TOOLS
  dnl never gets called
  AS_IF([test -z "$CC" && test x"$PETSC_HAVE_MPI" = x1 && test x"$PETSC_CC" != x],
        [CC="$PETSC_CC"])
...
# -------------------------------------------------------------
AC_DEFUN([LIBMESH_SET_BUILD_FLAGS],
[
  ACSM_SET_GLIBCXX_DEBUG_FLAGS
  ACSM_SET_BUILD_FLAGS
  ...
  SANITIZE_OPT_FLAGS="$ACSM_SANITIZE_OPT_FLAGS"
  SANITIZE_DBG_FLAGS="$ACSM_SANITIZE_DBG_FLAGS"
  SANITIZE_DEVEL_FLAGS="$ACSM_SANITIZE_DEVEL_FLAGS"
