...
[
  dnl We need to verify that we've done AC_ARG_ENABLE(mpi), AC_ARG_WITH(mpi), AC_ARG_ENABLE(petsc)
  dnl which occur in LIBMESH_COMPILER_CONTROL_ARGS
  AS_IF([test x"CALLED_COMPILER_CONTROL_ARGS" != x1],
        [AC_MSG_ERROR([the compiler control args macro must be called before setting compilers])])
  ...
  dnl Always query PETSc configure because we're going to use this later in libmesh_optional_packages
  CONFIGURE_PETSC
