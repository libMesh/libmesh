// $Id: mesh_ucd_support.C,v 1.1.1.1 2003-01-10 16:17:48 libmesh Exp $
	...
	for (unsigned int n=0; n<elem(i)->n_nodes(); n++)
	  {
	    ...
	    elem(i)->node(n) = (node-1); // assign the node, remember
	    // UCD is 1-based, so subtract
	  };
      };
  };  
};
...
void MeshBase::write_ucd (std::ostream &out)
{
  ...
  // Write header to stream
  {
	...
	<< "#" << std::endl
	<< "# $Id: mesh_ucd_support.C,v 1.1.1.1 2003-01-10 16:17:48 libmesh Exp $" << std::endl
	<< "#" << std::endl
  ...
  for (unsigned int n=0; n<n_nodes(); n++)
    {
      out << (n+1) << "\t";
      _vertices[n].write_unformatted(out);
    };
