...
public:
  ...
  void testSolution (const MeshBase & read_mesh, const System & sys2)
  {
    ...
    // Translation from a node's position on the generating 3x3 grid
    // to dof indexing order as gets done in serial.  We look nodes up
    // by position (rather than by id) because Nemesis reassigns node
    // ids on read based on partition ownership, so a read-back node's
    // id need not match the id it had when the file was written.
    ...
    for (const auto & node : read_mesh.node_ptr_range())
      {
        ...
        // The mesh is the 3x3 grid of nodes generated by
        // build_square(-1,1,-1,1), so each coordinate is one of
        // {-1,0,1}; map that directly to a 0-based grid index.
        const auto grid_index =
          cast_int<dof_id_type>(std::lround((*node)(0)) + 1 +
                                3*(std::lround((*node)(1)) + 1));

        CPPUNIT_ASSERT_LESS(node_reordering.size(), std::size_t(grid_index));
        const dof_id_type gold_i_ux = node_reordering[grid_index] * 3;
