...
Communicator *TestCommWorld;
  ...
  void testPushImpl(int M)
  {
    ...
    PassMess::push_parallel_vector_data(*TestCommWorld, data, collect_data);
              ...
              if (received_data.count(srcp))
                {
                  ...
                  PASSMESS_UNIT_ASSERT(std::count(datum.begin(), datum.end(), p) == std::ptrdiff_t(0));
                }
              continue;
            }

          PASSMESS_UNIT_ASSERT(received_data.count(srcp) == std::size_t(1));
          const std::vector<unsigned int> & datum = received_data[srcp];
          PASSMESS_UNIT_ASSERT(std::count(datum.begin(), datum.end(), p) == std::ptrdiff_t(diffsqrt+1));
          checked_sizes[srcp] += diffsqrt+1;
        }

    for (int srcp=0; srcp != size; ++srcp)
      PASSMESS_UNIT_ASSERT(checked_sizes[srcp] == received_data[srcp].size());
  }
  ...
  void testPullImpl(int M)
  {
    ...
    auto collect_replies =
      ...
      {
        const std::size_t query_size = query.size();
        PASSMESS_UNIT_ASSERT(query_size == response.size());
        for (unsigned int i=0; i != query_size; ++i)
          {
            PASSMESS_UNIT_ASSERT(query[i]*query[i] == response[i]);
          }
    ...
    PassMess::pull_parallel_vector_data
      (*TestCommWorld, data, compose_replies, collect_replies, ex);

    // Test the received results, for each query we sent.
    for (int p=0; p != M; ++p)
      {
        PASSMESS_UNIT_ASSERT(data[p].size() == received_data[p].size());
        for (std::size_t i = 0; i != data[p].size(); ++i)
          PASSMESS_UNIT_ASSERT(data[p][i]*data[p][i] == received_data[p][i]);
      }
  }
  ...
  void testPushVecVecImpl(int M)
  {
    ...
    auto collect_data =
       ...
       const typename std::vector<std::vector<unsigned int>> & data)
      {
        ...
        PASSMESS_UNIT_ASSERT(data.size() == std::size_t(2));
        PASSMESS_UNIT_ASSERT(data[1].size() == std::size_t(1));
        PASSMESS_UNIT_ASSERT(data[0][0] == data[1][0]);
      };
    ...
    auto collect_replies =
       ...
       const std::vector<std::vector<unsigned int>> & response)
      {
        ...
        for (unsigned int i=0; i != query_size; ++i)
          {
            const std::size_t query_i_size = query[i].size();
            PASSMESS_UNIT_ASSERT(query_i_size == response[i].size());
            for (unsigned int j=0; j != query_i_size; ++j)
              PASSMESS_UNIT_ASSERT(query[i][j]*query[i][j] == response[i][j]);
          }
        auto & vec = received_data[pid];
        vec.emplace_back(response[0].begin(), response[0].end());
        PASSMESS_UNIT_ASSERT(response[1].size() == std::size_t(1));
        PASSMESS_UNIT_ASSERT(response[1][0] == response[0][0]);
    ...
    for (int p=0; p != M; ++p)
      {
        ...
        for (std::size_t i = 0; i != data[p].size(); ++i)
          for (std::size_t j = 0; j != data[p][i].size(); ++j)
            PASSMESS_UNIT_ASSERT(data[p][i][j]*data[p][i][j] == received_data[p][i][j]);
      }
  }
          ...
          if (diffsqrt*diffsqrt != diffsize)
            {
              for (auto pv_it = rng.first; pv_it != rng.second; ++pv_it)
                {
                  PASSMESS_UNIT_ASSERT(std::count(pv_it->second.begin(), pv_it->second.end(), p) == std::ptrdiff_t(0));
                }
              continue;
            }

          PASSMESS_UNIT_ASSERT(rng.first != rng.second);
          for (auto pv_it = rng.first; pv_it != rng.second; ++pv_it)
            {
              std::ptrdiff_t cnt = std::count(pv_it->second.begin(), pv_it->second.end(), p);
              if (cnt)
                {
                  PASSMESS_UNIT_ASSERT(cnt == std::ptrdiff_t(diffsqrt+1));
                  auto pv_it2 = pv_it; ++pv_it2;
                  PASSMESS_UNIT_ASSERT(pv_it2 != rng.second);
                  std::ptrdiff_t cnt2 = std::count(pv_it2->second.begin(), pv_it2->second.end(), p);
                  PASSMESS_UNIT_ASSERT(cnt2 == std::ptrdiff_t(1));
    ...
    for (int srcp=0; srcp != size; ++srcp)
      {
        ...
        PASSMESS_UNIT_ASSERT(checked_sizes[srcp] == total_size);
      }
  }
              ...
              for (auto pvv = rng.first; pvv != rng.second; ++pvv)
                {
                  ...
                  PASSMESS_UNIT_ASSERT(std::count(v.begin(), v.end(), p) == std::ptrdiff_t(0));
                }
              continue;
            }
          ...
          for (auto pvv_it = rng.first; pvv_it != rng.second; ++pvv_it)
            {
              if(pvv_it->second.size() != std::size_t(2))
                passmess_error();
              PASSMESS_UNIT_ASSERT(pvv_it->second.size() == std::size_t(2));
              ...
              if (cnt)
                {
                  ...
                  PASSMESS_UNIT_ASSERT(pvv_it2 != rng.second);
                  PASSMESS_UNIT_ASSERT(pvv_it2->second.size() == std::size_t(1));
                  std::ptrdiff_t cnt3 = std::count(pvv_it2->second[0].begin(), pvv_it2->second[0].end(), p);
                  PASSMESS_UNIT_ASSERT(cnt3 == std::ptrdiff_t(1));
                  checked_sizes[srcp] += cnt + cnt2 + cnt3;
                  break;
                }
              ++pvv_it;
              passmess_assert(pvv_it != rng.second);
            }
        }
