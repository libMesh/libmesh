...
std::vector<std::string> pt_number;
  ...
  void testGatherString()
  {
    ...
    if (TestCommWorld->rank() == 0)
      for (processor_id_type i=0; i<vals.size(); i++)
        PASSMESS_UNIT_ASSERT( "Processor" + pt_number[i % 10]  == vals[i] );
  }
  ...
  void testAllGatherVectorString()
  {
    ...
    for (processor_id_type i=0; i<(vals.size()/2); i++)
      {
        PASSMESS_UNIT_ASSERT( "Processor" + pt_number[i % 10] + "A"  == vals[2*i] );
        PASSMESS_UNIT_ASSERT( "Processor" + pt_number[i % 10] + "B"  == vals[2*i+1] );
      }
  }
  ...
  void testAllGatherEmptyVectorString()
  {
    ...
    PASSMESS_UNIT_ASSERT( vals.empty() );
  }
  ...
  void testAllGatherHalfEmptyVectorString()
  {
    ...
    PASSMESS_UNIT_ASSERT( vals[0] == std::string("Proc 0 only") );
  }
  ...
  void testBroadcast()
  {
    ...
    for (std::size_t i=0; i<src.size(); i++)
      PASSMESS_UNIT_ASSERT( src[i]  == dest[i] );
  }
  ...
  void testBroadcastNestedType()
  {
    ...
    for (std::size_t i=0; i<src.size(); i++)
      {
        PASSMESS_UNIT_ASSERT(src[i].first.first.first.first ==
                             dest[i].first.first.first.first);
        PASSMESS_UNIT_ASSERT(src[i].first.first.first.second ==
                             dest[i].first.first.first.second);
        PASSMESS_UNIT_ASSERT(src[i].first.first.second ==
                             dest[i].first.first.second);
        PASSMESS_UNIT_ASSERT(src[i].first.second ==
                             dest[i].first.second);
        PASSMESS_UNIT_ASSERT(src[i].second ==
                             dest[i].second);
      }
  }
  ...
  void testScatter()
  {
    // Test Scalar scatter
    {
      ...
      PASSMESS_UNIT_ASSERT( TestCommWorld->rank() == dest );
    }
    ...
    // Test Vector Scatter (equal-sized chunks)
    {
      ...
      for (unsigned int i=0; i<CHUNK_SIZE; i++)
        PASSMESS_UNIT_ASSERT( TestCommWorld->rank() * CHUNK_SIZE + i == dest[i] );
    }
    ...
    // Test Vector Scatter (jagged chunks)
    {
      ...
      for (unsigned int i=0; i<=TestCommWorld->rank(); i++)
        PASSMESS_UNIT_ASSERT( start_value + i == dest[i] );
    }
  ...
  void testMin ()
  {
    ...
    PASSMESS_UNIT_ASSERT (min == static_cast<unsigned int>(0));
  }
  ...
  void testMax ()
  {
    ...
    PASSMESS_UNIT_ASSERT (cast_int<processor_id_type>(max+1) ==
  ...
  void testMinloc ()
  {
    ...
    PASSMESS_UNIT_ASSERT (min == static_cast<int>(0));
    PASSMESS_UNIT_ASSERT (minid == static_cast<unsigned int>(TestCommWorld->size()-1));
  }
  ...
  void testMaxloc ()
  {
    ...
    PASSMESS_UNIT_ASSERT (max+1 ==
                          cast_int<int>(TestCommWorld->size()));
    PASSMESS_UNIT_ASSERT (maxid == static_cast<unsigned int>(TestCommWorld->size()-1));
  }
  ...
  void testMinlocDouble ()
  {
    ...
    PASSMESS_UNIT_ASSERT (min == double(0));
    PASSMESS_UNIT_ASSERT (minid == static_cast<unsigned int>(TestCommWorld->size()-1));
  }
  ...
  void testMaxlocDouble ()
  {
    ...
    PASSMESS_UNIT_ASSERT (max+1 == double(TestCommWorld->size()));
    PASSMESS_UNIT_ASSERT (maxid == static_cast<unsigned int>(TestCommWorld->size()-1));
  }
  ...
  void testInfinityMin ()
  {
    ...
    PASSMESS_UNIT_ASSERT (min == std::numeric_limits<double>::infinity());
  ...
  void testInfinityMax ()
  {
    ...
    PASSMESS_UNIT_ASSERT (max == std::numeric_limits<double>::infinity());
  ...
  void testIsendRecv ()
  {
    ...
    PassMess::Request request;

    if (TestCommWorld->size() > 1)
      {
        // Default communication
        TestCommWorld->send_mode(PassMess::Communicator::DEFAULT);
        ...
        PassMess::wait (request);

        PASSMESS_UNIT_ASSERT ( src_val.size()  == recv_val.size() );

        for (std::size_t i=0; i<src_val.size(); i++)
          PASSMESS_UNIT_ASSERT( src_val[i]  == recv_val[i] );


        // Synchronous communication
        TestCommWorld->send_mode(PassMess::Communicator::SYNCHRONOUS);
    ...
    if (TestCommWorld->size() > 1)
      {
        ...
        for (std::set<unsigned int>::const_iterator
             ...
             ++it)
          PASSMESS_UNIT_ASSERT ( recv_val.count(*it) );
    ...
    if (TestCommWorld->size() > 1)
      {
        ...
        for (std::size_t i = 0; i != 3; ++i)
          PASSMESS_UNIT_ASSERT ( src_val[i].size() == recv_val[i].size() );

        PASSMESS_UNIT_ASSERT ( recv_val[0][0] == static_cast<unsigned int> (4) );
        PASSMESS_UNIT_ASSERT ( recv_val[2][0] == static_cast<unsigned int> (TestCommWorld->rank()) );
        PASSMESS_UNIT_ASSERT ( recv_val[2][1] == procdown );
  ...
  void testSemiVerify ()
  {
    ...
    PASSMESS_UNIT_ASSERT (TestCommWorld->semiverify(infptr));
  ...
  void testSplit ()
  {
    PassMess::Communicator subcomm;
    unsigned int rank = TestCommWorld->rank();
    unsigned int color = rank % 2;
    TestCommWorld->split(color, rank, subcomm);

    PASSMESS_UNIT_ASSERT(subcomm.size() >= 1);
    PASSMESS_UNIT_ASSERT(subcomm.size() >= TestCommWorld->size() / 2);
    PASSMESS_UNIT_ASSERT(subcomm.size() <= TestCommWorld->size() / 2 + 1);
  }
  ...
  void testSplitByType ()
  {
    ...
    PASSMESS_UNIT_ASSERT(subcomm.size() >= 1);
    PASSMESS_UNIT_ASSERT(subcomm.size() <= TestCommWorld->size());
  }
