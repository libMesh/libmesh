AC_DEFUN([SET_BUILD_METHODS],
[
 AC_ARG_VAR([METHODS], [methods used to build with, e.g. "opt dbg devel". Possibilities include: (opt,dbg,devel,prof,oprof)])
 ...
 AC_ARG_WITH(methods,
             AS_HELP_STRING([--with-methods=METHODS],
                            [methods used to build with (opt,dbg,devel,prof,oprof)]),
 ...
 AC_MSG_RESULT([<<< Configuring with methods "$METHODS" >>>])

 AC_ARG_VAR([passmess_CPPFLAGS], [User-specified C/C++ preprocessor flags])
 AC_ARG_VAR([passmess_CXXFLAGS], [User-specified C++ compilation flags])
 AC_ARG_VAR([passmess_CFLAGS],   [User-specified C compilation flags])
 ...
 CPPFLAGS_OPT="$CPPFLAGS_OPT $passmess_CPPFLAGS"
 CXXFLAGS_OPT="$CXXFLAGS_OPT $passmess_CXXFLAGS"
 CFLAGS_OPT="$CFLAGS_OPT $passmess_CFLAGS"

 CPPFLAGS_DBG="$CPPFLAGS_DBG $passmess_CPPFLAGS"
 CXXFLAGS_DBG="$CXXFLAGS_DBG $passmess_CXXFLAGS"
 CFLAGS_DBG="$CFLAGS_DBG $passmess_CFLAGS"

 CPPFLAGS_DEVEL="$CPPFLAGS_DEVEL $passmess_CPPFLAGS"
 CXXFLAGS_DEVEL="$CXXFLAGS_DEVEL $passmess_CXXFLAGS"
 CFLAGS_DEVEL="$CFLAGS_DEVEL $passmess_CFLAGS"
 ...
 AM_CONDITIONAL(BUILD_OPT_MODE,   test x$build_opt   = xyes)
 AM_CONDITIONAL(BUILD_DBG_MODE,   test x$build_dbg   = xyes)
 AM_CONDITIONAL(BUILD_DEVEL_MODE, test x$build_devel = xyes)
 AM_CONDITIONAL(BUILD_PROF_MODE,  test x$build_prof  = xyes)
 AM_CONDITIONAL(BUILD_OPROF_MODE, test x$build_oprof = xyes)
