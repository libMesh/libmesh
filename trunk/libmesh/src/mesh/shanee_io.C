// $Id: shanee_io.C,v 1.1 2004-03-23 05:16:22 jwpeterson Exp $
...
void ShaneeIO::read(const std::string& name)
{
...
void ShaneeIO::read_stream (std::istream &in)
{
    ...
    for (unsigned int i=0; i<nElem; i++)
      {
       ...
       // _elements[i]->set_id (i); needed?

	for (unsigned int n=0; n<elem->n_nodes(); n++)
	  {
	    in >> node; // read the current node
	    
	    elem->set_node(n) = mesh.node_ptr(node); // assign the node
	  }
      }
  }
}
