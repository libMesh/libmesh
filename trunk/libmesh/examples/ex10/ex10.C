...
{
  matrix_assembly.elapsed_time=0;
  matrix_assembly.count=0;
  error_est.elapsed_time=0;
  error_est.count=0;
...
public:
  AssembleMatrix (EquationSystems& es_in,
...
{  
    ...
    __range.reset (mesh.active_local_elements_begin(),
		   mesh.active_local_elements_end());

    tbb::tick_count t0 = tbb::tick_count::now();
    Threads::parallel_for (__range,				
			   AssembleMatrix (es, system_name)
			   );
    tbb::tick_count t1 = tbb::tick_count::now();
    matrix_assembly.elapsed_time += (t1-t0).seconds();
    matrix_assembly.count++;
}
