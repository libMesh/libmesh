// $Id: cell_pyramid5.C,v 1.1.1.1 2003-01-10 16:17:48 libmesh Exp $
...
std::auto_ptr<Elem> Pyramid5::build_side (const unsigned int i) const
{
  assert (i < n_sides());
...
unsigned int Pyramid5::vtk_element_type (const unsigned int sc) const
{
  return 14;
};
...
void Pyramid5::refine(Mesh& mesh)
{
  ...
  const Point p0_1 = (mesh.vertex(node(0))*.5 +
		      mesh.vertex(node(1))*.5  );
  
  const unsigned int n0_1 = mesh.mesh_refinement.add_node(p0_1, edge.key());
  

  edge.node(0) = node(1);
  edge.node(1) = node(2);
  
  const Point p1_2 = (mesh.vertex(node(1))*.5 +
		      mesh.vertex(node(2))*.5  );
  
  const unsigned int n1_2 = mesh.mesh_refinement.add_node(p1_2, edge.key());
  

  edge.node(0) = node(2);
  edge.node(1) = node(3);
  
  const Point p2_3 = (mesh.vertex(node(2))*.5 +
		      mesh.vertex(node(3))*.5  );
  
  const unsigned int n2_3 = mesh.mesh_refinement.add_node(p2_3, edge.key());
  

  edge.node(0) = node(0);
  edge.node(1) = node(3);
  
  const Point p0_3 = (mesh.vertex(node(0))*.5 +
		      mesh.vertex(node(3))*.5  );
  
  const unsigned int n0_3 = mesh.mesh_refinement.add_node(p0_3, edge.key());
  

  edge.node(0) = node(0);
  edge.node(1) = node(4);
  
  const Point p0_4 = (mesh.vertex(node(0))*.5 +
		      mesh.vertex(node(4))*.5  );
  
  const unsigned int n0_4 = mesh.mesh_refinement.add_node(p0_4, edge.key());
  

  edge.node(0) = node(1);
  edge.node(1) = node(4);
  
  const Point p1_4 = (mesh.vertex(node(1))*.5 +
		      mesh.vertex(node(4))*.5  );
  
  const unsigned int n1_4 = mesh.mesh_refinement.add_node(p1_4, edge.key());
  

  edge.node(0) = node(2);
  edge.node(1) = node(4);
  
  const Point p2_4 = (mesh.vertex(node(2))*.5 +
		      mesh.vertex(node(4))*.5  );

  const unsigned int n2_4 = mesh.mesh_refinement.add_node(p2_4, edge.key());
  

  edge.node(0) = node(3);
  edge.node(1) = node(4);
  
  const Point p3_4 = (mesh.vertex(node(3))*.5 +
		      mesh.vertex(node(4))*.5  );
  
  const unsigned int n3_4 = mesh.mesh_refinement.add_node(p3_4, edge.key());
  ...
  const Point p0 = (mesh.vertex(node(0)) +
		    mesh.vertex(node(3)) +
		    mesh.vertex(node(2)) +
		    mesh.vertex(node(1)))*.25;

  const unsigned int n0 = mesh.mesh_refinement.add_node(p0, face.key());
