...
void create_random_point_cloud(const unsigned int Npts, std::vector<Point> &pts,
...
int main(int argc, char **argv) {
  ...
  LibMeshInit init(argc, argv);
  {
    ...
    // Demonstration case 1
    {
      ...
      create_random_point_cloud(my_n_source_points, idi.get_source_points());
      ...
      // Explicitly set the data values we will interpolate from
      {
        ...
        src_vals.clear();
        src_vals.reserve(2 * src_pts.size());
      ...
      // Interpolate to some other random points, and evaluate the result
      {
        ...
        create_random_point_cloud(n_target_points, tgt_pts);

        // tgt_pts = rbi.get_source_points();

        idi.interpolate_field_data(field_vars, tgt_pts, tgt_data_idi);

        rbi.interpolate_field_data(field_vars, tgt_pts, tgt_data_rbi);

        std::vector<Number>::const_iterator v_idi = tgt_data_idi.begin(),
        ...
        for (std::vector<Point>::const_iterator p_it = tgt_pts.begin();
          ...
          libMesh::out << "\n v_interp_idi=" << *v_idi
                       << ", v_interp_rbi=" << *v_rbi
                       << ", v_exact=" << exact_solution_v(*p_it) << std::endl;
          ++v_idi;
          ++v_rbi;
        }
      }
    ...
    // Demonstration case 2
    {
      ...
      TecplotIO(mesh_a).write_equation_systems("src.dat", es_a);
      ...
      // We have only set local values - prepare for use by gathering remote
      // data
      idi.prepare_for_use();
      rbi.prepare_for_use();
      ...
      // also provide a locking mechanism.
      {
        ...
        // Write the result
        TecplotIO(mesh_b).write_equation_systems("dest_idi.dat", es_b);
      }
      ...
      // mechanism.
      {
        ...
        // Write the result
        TecplotIO(mesh_b).write_equation_systems("dest_rbi.dat", es_b);
      }
    }
  }
  return 0;
