  ...
  _mesh(m)
{
  ...
  // fill the _initial_nodes vector
  MeshBase::node_iterator it = _mesh.nodes_begin();
  const MeshBase::node_iterator end = _mesh.nodes_end();

  // Do not create a MeshRefinement object if the Mesh
  // is not yet initialized (i.e. read in or built by one
  // of the Generation methods.)
  //
  // Note that if you create a Mesh, then construct a MeshRefinement
  // object, and then _change_ the Mesh, (i.e. clear or read in a new one) the
  // MeshRefinement object may no longer be in a consistent state. 
  if (it == end)
  {
    std::cerr << "Mesh object is not yet initialized (contains no nodes). \n"
              << "Construct the MeshRefinement object only after the Mesh is \n"
              << "initialized (i.e. read in or generated by build routines.)"
              << std::endl;
    error();
  }
...
Node* MeshRefinement::add_point (const Point& p, const unsigned int key)
{
  ...
  // Before searching the new_nodes map, search the vector of 
  // initial nodes for p.
