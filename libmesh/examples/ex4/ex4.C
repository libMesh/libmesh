...
namespace {
  class AssemblePoisson
  { 
  private:
    EquationSystems& _es;
    const std::string& _system_name;
    Threads::spin_mutex &_matrix_mutex;
    Threads::spin_mutex &_rhs_mutex;
    
  public:
    AssemblePoisson (EquationSystems& es,
		     const std::string& system_name,
		     Threads::spin_mutex &matrix_mutex,
		     Threads::spin_mutex &rhs_mutex) :
      _es(es),
      _system_name(system_name),
      _matrix_mutex(matrix_mutex),
      _rhs_mutex(rhs_mutex)
    {}
    
    void operator()(const ConstElemRange &range) const
    {
      EquationSystems &es=_es;
      const std::string &system_name = _system_name;
      ...
      for (ConstElemRange::const_iterator el=range.begin() ; el != range.end(); ++el)
	{
	  ...
	  {
	    Threads::spin_mutex::scoped_lock lock(_matrix_mutex);
	    system.matrix->add_matrix (Ke, dof_indices);
	  }
	  {
	    Threads::spin_mutex::scoped_lock lock(_rhs_mutex);
...
void assemble_poisson(EquationSystems& es,
                      const std::string& system_name)
{
  // Create some mutual exclusion objects to manage
  // access to the shared global matrix and right-hand-side.
  Threads::spin_mutex matrix_mutex, rhs_mutex;
  // Get a constant reference to the mesh object.
  const MeshBase& mesh = es.get_mesh();

  Threads::parallel_for (ConstElemRange(mesh.local_elements_begin(),
